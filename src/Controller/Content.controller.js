...
const getAllSection = async (req, res) => {
    try {
        const SectionAll = await Section.find();

        if (!SectionAll) {
            return res.status(400).json({ data: null, meassage: "AllSection Not added" })
        }

        return res.status(200).json({ data: SectionAll, meassage: "AllSection added Sucessfully" })
    } catch (error) {
        return res.status(500).json({ data: null, meassage: "Incress Not define AllSection" + error.meassage })
    }
}
